...
def _export_row(
    row: dict[str, object], average_traded_value_5d: Decimal | None = None
) -> dict[str, object]:
...
def _write_xlsx(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    for column in numeric_columns:
        for cell in sheet.iter_cols(min_col=column, max_col=column, min_row=2):
            cell[0].number_format = "#,##0.00"
    widths = (16, 18, 15, 20, 18, 22, 19, 18, 15, 23, 24, 34, 25, 18)
...
async def main() -> int:
    ...
    rows = _rank_rows(
        [_export_row(asdict(item), average_values.get(item.symbol)) for item in opportunities]
    )
    if args.output and rows:
        if args.output.suffix.lower() == ".xlsx":
            _write_xlsx(args.output, rows)
